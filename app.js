...
/* No built-in parent types — ALL parents are user-created */
...
function applyRoleUI() {
  // Set body role class for CSS mobile-nav role filtering
  document.body.classList.remove('role-ADMIN','role-MANAGER','role-VIEWER');
  if(S.user?.role) document.body.classList.add('role-'+S.user.role);
...
function goTo(sec) {
  ...
  document.querySelectorAll('.mbn-item').forEach(el => el.classList.toggle('active', el.dataset.sec === sec));
...
function updateStats() {
  ...
  items.forEach(i => {
    ...
    const threshold = parseInt(i.lowStockThreshold) || 3;
    if(qty > 0 && qty <= threshold) lowStockCount++;
  });

  const salesArr  = Object.values(S.sales);
  const totalSold = salesArr.reduce((s,r) => s + (parseInt(r.quantitySold)||0), 0);
  const unpaidCount = salesArr.filter(r => !r.paid).length;

  // Render dynamic parent category stat cards
  renderParentStatCards(qtyByType);

  setText('st-total',    totalQty);
  setText('st-sold',     totalSold);
  setText('st-lowstock', lowStockCount);
  setText('st-unpaid',   unpaidCount);
...
function renderParentStatCards(qtyByType) {
  const container = document.getElementById('parent-stat-cards');
  if(!container) return;
  const allTypes = getAllParentTypes();
  const dotColors = ['blue','green','amber','red','purple','teal'];
  container.innerHTML = allTypes.map((type, idx) => {
    const dotCls = dotColors[idx % dotColors.length];
    return `<div class="stat-card s-dynamic" style="--dyn-dot:var(--${dotCls === 'blue' ? 'blue' : dotCls === 'green' ? 'green' : dotCls === 'amber' ? 'amber' : 'red'}, #6366f1)">
      <div class="stat-icon-wrap"><span style="font-size:1.4rem">${type.icon}</span></div>
      <div>
        <div class="stat-num">${qtyByType[type.key] || 0}</div>
        <div class="stat-lbl">${esc(type.label)}</div>
      </div>
    </div>`;
  }).join('');
}
...
function renderUnpaidSales() {
  const el = document.getElementById('unpaid-sales-list'); if(!el) return;
  const unpaid = Object.entries(S.sales)
    .filter(([,s]) => !s.paid)
    .sort((a,b) => b[1].timestamp - a[1].timestamp);
  if(!unpaid.length) {
    el.innerHTML='<div class="empty-state-sm" style="color:var(--green)">✓ All sales are paid</div>';
    return;
  }
  const canPay = S.user?.role === 'ADMIN' || S.user?.role === 'MANAGER';
  el.innerHTML = `<table class="dtable" style="font-size:.82rem">
    <thead><tr><th>Item</th><th>Vendor/Customer</th><th>Qty</th><th>Date</th><th>Action</th></tr></thead>
    <tbody>` +
    unpaid.map(([saleId,s]) => `
      <tr>
        <td><strong>${esc(s.itemName||'—')}</strong></td>
        <td>${esc(s.vendorName||'—')}</td>
        <td style="font-family:var(--fmono)">${s.quantitySold||0}</td>
        <td style="color:var(--tx3)">${fmtDate(s.saleDate)}</td>
        <td>${canPay ? `<button class="btn btn-primary" style="padding:.2rem .7rem;font-size:.75rem" onclick="togglePaid('${saleId}',true)">Mark Paid</button>` : '—'}</td>
      </tr>`).join('') +
    `</tbody></table>`;
}
...
async function submitManual() {
  ...
  const lowStock = parseInt(document.getElementById('fm-lowstock')?.value) || 3;

  // ── Check for existing item with same name + type + category ──
  const nameLower = name.toLowerCase();
  const duplicate = Object.entries(S.stock).find(([,i]) =>
    i.itemName?.toLowerCase() === nameLower &&
    i.parentType === type &&
    i.category   === cat
  );
  ...
  try {
    if(duplicate) {
      // Merge: add qty to existing record, update status/notes/threshold if provided
      const [dupKey, dupItem] = duplicate;
      const newQty = (parseInt(dupItem.quantity) || 0) + qty;
      const updates = {
        quantity:         newQty,
      ...
      await fbUpdate(`${DB.STOCK}/${dupKey}`, updates);
      await logActivity(`Updated "${name}" stock +${qty} → ${newQty} total`);
      showEl(okEl, `✓ "${name}" already exists — stock updated from ${dupItem.quantity} → ${newQty} units.`);
    } else {
      // New item
      const item = {
        itemName:name, serialNumber:serial||genId('SRA'), parentType:type,
        category:cat, quantity:qty, status, lowStockThreshold:lowStock,
      ...
      showEl(okEl, `✓ "${name}" added successfully!`);
    }
...
function refreshItemNameSuggestions() {
  const dl = document.getElementById('item-name-suggestions');
  if(!dl) return;
  // Deduplicate names from all stock items
  const names = [...new Set(Object.values(S.stock).map(i => i.itemName).filter(Boolean))].sort();
  dl.innerHTML = names.map(n => `<option value="${esc(n)}"></option>`).join('');
}

function clearManual() {
  ['fm-name','fm-serial','fm-notes'].forEach(id => val(id,''));
  val('fm-qty','1'); val('fm-lowstock','3'); val('fm-type',''); val('fm-status','AVAILABLE'); val('fm-date',today());
...
function openSell(id) {
  ...
  loadVendorSuggestions();
  // reset paid checkbox
  const pc = document.getElementById('sell-paid'); if(pc) pc.checked = false;
  openModal('m-sell');
}
...
/* ── VENDOR NAME MEMORY ───────────────────────────────────────── */
async function saveVendorName(name) {
  if(!name) return;
  try {
    const key = name.toLowerCase().replace(/[^a-z0-9]/g,'_').substring(0,40);
    await fbSet(`${DB.VENDORS}/${key}`, { name, updatedAt: Date.now() });
    loadVendorSuggestions();
  } catch(_) {}
}
...
async function loadVendorSuggestions() {
  try {
    const data = await fbRead(DB.VENDORS);
    const dl   = document.getElementById('vendor-suggestions');
    if(!dl || !data) return;
    dl.innerHTML = Object.values(data)
...
async function togglePaid(saleId, paid) {
  try {
    await fbUpdate(`${DB.SALES}/${saleId}`, { paid });
...
function renderSalesTable() {
  ...
  if(!rows.length) {
    tbody.innerHTML='<tr><td colspan="9" class="tbl-empty">No sales records found.</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = rows.map(([saleId,s]) => `
    <tr class="${s.paid ? '' : 'unpaid-row'}">
      ...
      <td>
        ${canPay ? `<label class="paid-toggle" title="${s.paid ? 'Paid — click to mark unpaid' : 'Unpaid — click to mark paid'}">
          <input type="checkbox" ${s.paid ? 'checked' : ''} onchange="togglePaid('${saleId}', this.checked)" style="accent-color:var(--green)"/>
          <span class="paid-lbl ${s.paid ? 'paid' : 'unpaid'}">${s.paid ? '✓ Paid' : '✗ Unpaid'}</span>
        </label>` : `<span class="paid-lbl ${s.paid ? 'paid' : 'unpaid'}">${s.paid ? '✓ Paid' : '✗ Unpaid'}</span>`}
      </td>
      <td>
        ...
        <tr>
          ...
          <td>
            <div class="row-actions">
              ...
              <button class="ibtn edit" title="Edit Role" onclick="openEditRole('${esc(u.username||'')}','${esc(u.displayName||'')}','${esc(u.role||'')}')">
...
function openEditRole(username, displayName, currentRole) {
  if(S.user?.role !== 'ADMIN') { toast('Only admins can change roles.','error'); return; }
  val('er-username', username);
  val('er-display',  displayName);
  val('er-role',     currentRole);
  hideEl(document.getElementById('er-err'));
  openModal('m-edit-role');
}

async function saveEditRole() {
  const username = document.getElementById('er-username').value;
  const role     = document.getElementById('er-role').value;
  const errEl    = document.getElementById('er-err');
  hideEl(errEl);
  if(!role) { showEl(errEl,'Please select a role.'); return; }
  try {
    await fbUpdate(`${DB.USERS}/${username}`, { role });
    await logActivity(`Role of "${username}" changed to ${role}`);
    closeModal('m-edit-role');
    loadUsers();
    toast(`Role updated to ${role.replace('_',' ')} for "${username}".`,'success');
  } catch(e) { showEl(errEl,'Error: '+e.message); }
}

/* ── SELF CHANGE PASSWORD (any user, requires current pw) ─────── */
function openSelfChangePw() {
  ['sp-current','sp-new','sp-confirm'].forEach(id => val(id,''));
  hideEl(document.getElementById('sp-err'));
  hideEl(document.getElementById('sp-ok'));
  openModal('m-self-pw');
}
...
async function saveSelfPw() {
  ...
  if(!current)            { showEl(errEl,'Current password is required.'); return; }
  if(!newPw)              { showEl(errEl,'New password is required.'); return; }
  if(newPw.length < 6)    { showEl(errEl,'New password must be at least 6 characters.'); return; }
  if(newPw !== confirm)   { showEl(errEl,'Passwords do not match.'); return; }

  try {
    const username  = S.user.username;
    const userRec   = await fbRead(`${DB.USERS}/${username}`);
    const currentHash = await hashPw(current);
    if(currentHash !== userRec.pwHash) {
      showEl(errEl,'Current password is incorrect.'); return;
    }
    const newHash = await hashPw(newPw);
    await fbUpdate(`${DB.USERS}/${username}`, { pwHash: newHash });
    await logActivity(`Password changed by user "${username}"`);
    showEl(okEl,'✓ Password updated successfully!');
    ['sp-current','sp-new','sp-confirm'].forEach(id => val(id,''));
    setTimeout(() => closeModal('m-self-pw'), 1500);
...
══════════════════════════════════════ */
function toggleMbnMore() {
  const popup    = document.getElementById('mbn-more-popup');
  const backdrop = document.getElementById('mbn-more-backdrop');
...
function closeMbnMore() {
  document.getElementById('mbn-more-popup').classList.add('hidden');
  document.getElementById('mbn-more-backdrop').classList.add('hidden');
  const btn = document.getElementById('mbn-more-btn');
